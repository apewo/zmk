...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_SPLIT_MODE_STATUS)
static struct zmk_widget_split_mode_status split_mode_status_widget;
#endif
...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_SPLIT_MODE_STATUS)
    zmk_widget_split_mode_status_init(&split_mode_status_widget, screen);
    lv_obj_set_style_text_font(zmk_widget_split_mode_status_obj(&split_mode_status_widget),
                               lv_theme_get_font_small(screen), LV_PART_MAIN);
    lv_obj_align(zmk_widget_split_mode_status_obj(&split_mode_status_widget), LV_ALIGN_BOTTOM_RIGHT, 0, 0);
#endif
